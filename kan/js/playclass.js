...
function PlayHistoryClass() {
	...
	this.getPlayArray = function() {
		...
		var start = cookieStr.indexOf("gxl_ting_v=") + "gxl_ting_v=".length, end = cookieStr
				...
				if(result['rcode']>-1){
					...
					if(result['r']!=null&&result['r'].length>0){
						...
						for(var i in result['r']){
							...
							innerStr += "<li><h5><a href=\"" + data['ting_readurl'] 
									 + "\">"+data['ting_name']
							         +"</a><em>/</em><a href='"
							         +data['ting_palyurl']+"' target='_blank'>"
							         +data['url_name']+"</a></h5><label><a class=\"color\" href=\"" 
							         + data['ting_palyurl'] + "\">继续听</a></label><a href=\"javascript:;\" target='_blank' class='delck' data=\""
				...
				for ( var i = nameArray.length - 1; i >= 0; i--) {
					...
					if (vodnames.length == 2 && vodlinks.length ==2) {
						innerStr += "<li><h5><a href=\""
								...
								+ vodlinks[1]
								+ "\">继续听</a></label><a href=\"javascript:;\" class='delck' data='"
	...
	this.removeHistory = function(ii, type) {
		...
		if (type == 'inck') {
			...
			if (cookieStr.indexOf("gxl_ting_v=") != -1 || cookieStr.indexOf("_$_|") != -1) {
				var newCookieStr = "";
				...
				document.cookie = "gxl_ting_v=" + newCookieStr + timeAndPathStr;
				return true;
			}
			return false;
		}else{
	...
	this.addPlayHistory = function(json,ting_readurl,ting_palyurl) {
		var count = 10; // 播放历史列表调用条数
		if(checkcookie()){
			$.post(Root + "index.php?s=User-Comm-addplaylog",json);
		}
		var name = json.ting_name+"|" + ting_readurl ;
		var url = json.url_name  +"|" + ting_palyurl ;
		...
		if ((cookieStr.indexOf("gxl_ting_v=") != -1 || cookieStr.indexOf("_$_|") != -1) && allVideoArray != undefined) {
			var newCookieStr = "";
			...
			document.cookie = "gxl_ting_v=" + newCookieStr + code_name + code_url + timeAndPathStr;
		} else {
			document.cookie = "gxl_ting_v=" + code_name + code_url + timeAndPathStr;
		}
	}
...
function _GC() {
	...
	document.cookie = "gxl_ting_v=; path=/";
}
