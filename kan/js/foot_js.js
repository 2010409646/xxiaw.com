...
function clearcookie() {
	document.cookie = "qy_username=; path=/;domain=gxlcms.com;expires=" + new Date(1970, 1, 1).toGMTString(), document.cookie = "qy_userpwd=; path=/;domain=gxlcms.com;expires=" + new Date(1970, 1, 1).toGMTString(), document.cookie = "qy_userid=; path=/;domain=gxlcms.com;expires=" + new Date(1970, 1, 1).toGMTString()
}
function userreg() {
...
pp = null, $.extend({
	...
	a = a || {}, $.extend(b, a), null != a.msg && void 0 != a.msg && $("#ui-cnt").html(a.msg), 1 == parseInt(a.code) ? $("#ui-cnt").removeClass("loading error alert").addClass("succ") : parseInt(a.code) < 1 && $("#ui-cnt").removeClass("loading alert succ").addClass("error"), pp = setTimeout("$.closefloatdiv()", b.intvaltime)
}, function(a) {
	...
	$.get(Root + "index.php?s=User-Center-flushinfo", "", function(a) {
		return -7 == parseInt(a.code) ? ($.showfloatdiv({
			...
			code: -1,
			msg: a.msg
		}), !1) : (a.uid > 0 && (qruser = a, parseInt(a.history) > 5 ? ($("#morelog").html('<a target="_blank" href="index.php?s=User-Center-playlog">播放记录&gt;&gt;</a>'), $("#morelog").show()) : ($("#morelog").html(""), $("#morelog").hide()), loginhtml = $("#loginbarx").html(), $("#loginbarx").html(a.html), $("#his-todol").hide(),$("#loginbar").hide(), $(".logoutbt").unbind(), $(".logoutbt").click(function() {
