...
class ColRunModel extends AdvModel
{
	...
	public function Collecting(&$config)
	{
		...
		if (!empty($list) && is_array($list)) {
			...
			if ($page <= $total_page) {
				...
				F("_gxlnews/ColCache", $CacheUrl);
			}

			return $result;
		}
		else {
			if (F("_gxlnews/ColCache")) {
				F("_gxlnews/ColCache", NUll);
			}
	...
	public function Inflow($act)
	{
		...
		foreach ($ArrID as $key => $val ) {
			...
			preg_match_all("|/videos/(.*)vod-play-id-(.*)-|U", $data["playurl"], $id);
			...
			$data["vod_play"] = $playurls["players"];
			$data["vod_url"] = $playurls["urls"];

			if (!$this->CModel->videoImport($Cont["url"], $data, $Cont["nid"])) {
				$result .= "[" . $data["cname"] . "]<strong>" . $data["vod_name"] . "</strong>";
	...
	{
		$result = array();
		$arr = array("title" => "vod_name", "intro" => "vod_title", "time" => "vod_addtime", "mcid" => "vod_mcid", "director" => "vod_director", "actor" => "vod_actor", "content" => "vod_content", "picurl" => "vod_pic", "area" => "vod_area", "language" => "vod_language", "year" => "vod_year", "serial" => "vod_continu", "cid" => "vod_cid", "cname" => "cname", "reid" => "reid");
	...
	public function MyCollecting(&$config)
	{
		...
		if (!empty($list) && is_array($list)) {
			foreach ($list as $v ) {
				...
				preg_match_all("|/videos/(.*)vod-play-id-(.*)-|U", $html["playurl"], $id);
				...
				$html["vod_play"] = $playurls["players"];
				$html["vod_url"] = $playurls["urls"];
