...
class CustomCollectModel extends Model
{
	...
	{
		$data = array("title" => "影片名称", "cname" => "栏目分类", "intro" => "备注", "time" => "更新时间", "mcid" => "关键词", "director" => "导演", "actor" => "主演", "content" => "剧情介绍", "picurl" => "图片", "area" => "地区分类", "language" => "语言/对白", "year" => "上映时间", "serial" => "连载信息");
		return $data;
	}
	...
	public function ChannelList($where)
	{
		...
		$CacheData = F("_gxlnews/channel_999");
	...
	public function videoImport($url, $html, $nid = "")
	{
		...
		if (!$check) {
			...
			if (!empty($html["cname"]) && (empty($html["vod_cid"]) || ($html["vod_cid"] == 999))) {
				if ($html["vod_cid"] == 999) {
					...
					$html["vod_cid"] = getlistid($html["cname"]);
				}

				if (!$html["vod_cid"] || ($html["vod_cid"] == 999)) {
	...
	public function VideoSave(&$html, $nid, $id = "")
	{
		...
		if (!empty($id)) {
			$data["vod_name"] = $html["vod_name"];
			$data["vod_title"] = $html["vod_title"];
			$data["vod_keywords"] = $html["vod_keywords"];
			$data["vod_cid"] = $html["vod_cid"];
			$data["vod_continu"] = $html["vod_continu"];
			$data["vod_play"] = $html["vod_play"];
			$data["vod_server"] = $html["vod_server"];
			$data["vod_url"] = $html["vod_url"];
			$data["vod_addtime"] = time();
			$data["vod_reurl"] = $html["vod_reurl"];
			$data["vod_letter"] = getfirstchar($html["vod_name"]);

			if ($this->VdoDB->create($data)) {
				$sid = $this->VdoDB->where("vod_id=" . $id)->data($data)->save();
			...
			$html["vod_addtime"] = time();
			$html["vod_stars"] = 1;
			$html["vod_letter"] = getfirstchar($html["vod_name"]);
			$html["vod_hits"] = 0;
			$html["vod_gold"] = 0;
			$html["vod_golder"] = 0;
			$html["vod_up"] = 0;
			$html["vod_down"] = 0;
			$html["vod_inputer"] = "互联网采集";
			$html["vod_mcid"] = D("Mcat")->getmcid($html["vod_cid"], $html["vod_mcid"]);
			$html["vod_letters"] = $this->getletters($html);
	...
	public function getletters($vod)
	{
		$where = array();
		$vod_letters = gxl_pinyin($vod["vod_name"]);
		$where["vod_letters"] = $vod_letters;

		if (0 < $this->VdoDB->where($where)->count()) {
			$vod_letters = gxl_pinyin($vod["vod_name"], 1);
			$where["vod_letters"] = vod_letters;
			$i = 1;

			while (0 < $this->VdoDB->where($where)->count()) {
				$vod_letters = gxl_pinyin($vod["vod_name"], 1) . $i;
				$where["vod_letters"] = $vod_letters;
				$i++;
			}
		}
	...
	{
		if (empty($html["vod_name"]) || empty($html["vod_url"])) {
			$this->error = "影片名称或播放地址为空!";
			return false;
		}

		$html["vod_name"] = str_replace(array("HD", "BD", "DVD", "VCD", "TS", "【完结】", "【】", "[]", "()"), "", $html["vod_name"]);
		$html["vod_actor"] = str_replace(array(",", "/", "，", "|", "、"), " ", $html["vod_actor"]);
		$html["vod_director"] = str_replace(array(",", "/", "，", "|", "、"), " ", $html["vod_director"]);
		$ArrUrl = $this->VdoDB->where("vod_reurl='" . $url . "'")->find();

		if (!$ArrUrl) {
			$ArrTitle = $this->VdoDB->where("vod_name='" . $html["vod_name"] . "'")->find();

			if ($ArrTitle) {
				if ($this->InfoCheck($html, $ArrTitle)) {
					if ($this->BaseCheck($ArrTitle, $html)) {
						return $ArrTitle["vod_id"];
					}
			...
			if (C("play_collect_name")) {
				$len = ceil(strlen($html["vod_name"]) / 3) - intval(C("play_collect_name"));

				if (2 <= $len) {
					$like = msubstr($html["vod_name"], 0, $len);
					$where["vod_name"] = array("like", "%" . $like . "%");
					$ArrLike = $this->VdoDB->where($where)->find();
					if (!empty($html["vod_actor"]) && !empty($ArrLike["vod_actor"])) {
						$arr_actor_1 = explode(" ", $html["vod_actor"]);
						$arr_actor_2 = explode(" ", str_replace(array(",", "/", "，", "|", "、"), " ", $ArrLike["vod_actor"]));
						if (!array_digxl($arr_actor_1, $arr_actor_2) && !array_digxl($arr_actor_2, $arr_actor_1)) {
							if ($this->BaseCheck($ArrLike, $html)) {
								return $ArrLike["vod_id"];
							}
						}
					}

					if ($ArrLike && ($ArrLike["vod_inputer"] != "互联网采集")) {
						return "status";
					}
	...
	{
		if (empty($html["vod_actor"])) {
			return true;
		}

		if ($Arr["vod_actor"] == $html["vod_actor"]) {
			return true;
		}

		$arr_actor_1 = explode(" ", $html["vod_actor"]);
		$arr_actor_2 = explode(" ", str_replace(array(",", "/", "，", "|", "、"), " ", $Arr["vod_actor"]));
