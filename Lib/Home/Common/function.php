...
function get_actor_url($str, $num, $keyword, $classname)
{
	...
	foreach ($arr as $key => $val ) {
		...
		if (!empty($list["star_id"])) {
			$restr .= "<a  href=\"" . ff_star_url("read", $list["star_id"], $list["star_pyname"], 1) . "\" target=\"_blank\">" . $value . "</a>";
		}
		else {
			$restr .= "<a href=\"" . UU("Home-vod/search", array("wd" => urlencode(trim($val))), true, false) . "\" target=\"_blank\">" . $value . "</a> ";
		}
		if (($key + 1) == $num) {
			break;
		}
