...
class SpecialAction extends BaseAction{	
	...
	public function showdown($did,$tid){
		$where = array();
		$where['topic_sid'] = 3;
		$where['topic_tid'] = $tid;
		$rs = D('Topic');
		...
		//
		$rss = D('TopicdownView');
		$list = $rss->field('down_id,down_name,topic_oid')->where($where)->order('topic_oid desc,topic_did desc')->select();
		...
		if(!$list){
			...
			$this->assign('max_oid',$maxoid);
			$this->assign('min_oid',$minoid);
			$this->assign('list_down',$list);
			$this->assign('count',count($list));
			$this->display('./Public/system/special_down_ids.html');
		}
	}
	// Ajax设置星级
