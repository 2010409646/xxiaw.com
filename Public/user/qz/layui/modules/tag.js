...
layui.define('jquery', function(exports){
  ...
  TAG_ITEM ='tag-item',
  CLOSE = 'tag-close',
  DEFAULT_SKIN ='layui-btn layui-btn-primary layui-btn'
  ,tag = function(){
    this.config = {
  ...
  var call = {
    ...
    ,input: function(e, othis){
      ...
      //标签输入框
      var inpatNewTag = $('<div class="' + INPUT_NEW_TAG + '"><input type="text" autocomplete="off" class="layui-inputs"></div>');
      inpatNewTag.addClass(options.skin);
      buttonNewTag.after(inpatNewTag).remove();
      inpatNewTag.children('.layui-inputs').on('blur', function () {
        if(this.value){
          var options = {
            text: this.value
          }
